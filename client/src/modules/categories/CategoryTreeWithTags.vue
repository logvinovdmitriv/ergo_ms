    ...
    >
      <div class="d-flex align-items-center justify-content-between">
        <div>
          ...
          <span v-if="categoryTags(cat.id).length" class="ms-2 badge bg-primary">
            Теги: {{ categoryTags(cat.id).map(t => t.name).join(', ') }}
          </span>
        </div>
        <div>
...
function categoryTags(categoryId) {
  return props.tags.filter(t => t.category === categoryId)
}
</script>
<style scoped>
